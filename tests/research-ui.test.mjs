...
function staticContract() {
  ...
  assert(html.includes('--bg: #0b0d10'), "dark ink palette");
  assert(html.includes('--signal: #3ecfbe'), "signal teal accent");
  assert(html.includes("Instrument Serif"), "display serif");
  assert(html.includes(".trace-wrap.is-running"), "running class guard");
  assert(/\.trace-wrap\.is-running[\s\S]*?animation:\s*none/.test(html), "no entrance fade while running");
  ...
  assert(html.includes('content: "DOSSIER"'), "dossier folio stamp");
